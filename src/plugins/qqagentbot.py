...
from src.plugins.public import *
...
def load_memory(group_id,current_user_id,current_user_name,current_user_card,bot):
    """
    加载100条群聊信息
    ...
    if not history or len(history) == 0:
        ...
        result.append("【重要】你的QQ号:")
        result.append(f"- QQ号: {bot.self_id}")
        result.append("")
...
'''async def retrieve_context(query: str):
    ...
    score_threshold=1.52##归一化的相关性，大于1.52的都完全不相关
    results = results = [(doc, score) for doc, score in raw_results if score <= score_threshold]
...
async def retrieve_context_yuanshiindex(query: str):
    """
    加载专属知识库。
...
agent = create_agent(
    ...
    middleware=[first_layer_security,testafter],
)
